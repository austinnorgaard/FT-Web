...
export class LoginComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.loggedOutType === "logged out"){
            this.showErrorMessage("Successfully Logged Out", "ng-alert");
    ...
    async login() {
        ...
        if (!credentials.isValid()) {
            this.showErrorMessage("Enter an email and password", null);
        ...
        if (result === LoginResult.Success) {
            ...
            this.showErrorMessage("Incorrect email or password", null);
        } else if (result === LoginResult.FailureNetwork) {
            this.showErrorMessage("Network or client error. Internet connectivity, or server down?", null);
        } else if (result === LoginResult.FailureServer) {
            this.showErrorMessage("Login failure due to server error", null);
        } else if (result === LoginResult.Unknown) {
            this.showErrorMessage("Login failure due to unknown error", null);
        }
        this.loginInProgress = false;
    }
    ...
    showErrorMessage(message: string, styling: string) {
        styles: [styling];
